...
def main(argv=None):
    ...
    config_file = "experiments/multi_network_SHG_lv2_conv2.cfg"
    if len(argv) != 1:
        config_file = argv[1]
    ...
    with tf.Graph().as_default(), tf.device("/cpu:0"):
        ...
        pred_result_image = tf.placeholder(tf.float32, shape=[params['batchsize']//4, 480, 640, 3])
        pred_result__summary = tf.summary.image("pred_result_image", pred_result_image, params['batchsize']//4)
        ...
        with tf.Session(config=config) as sess:
            ...
            if params['checkpoint']:
                ...
                print('Restore from checkpoint...Done')
            ...
            for step in range(START_STEP, total_step_num):
                ...
                if step != 0 and step % params['per_update_tensorboard_step'] == 0 or step==1:
                ...
                if step % params['per_saved_model_step'] == 0 :
                    ...
                    if params['pred_image_on_tensorboard'] is True:
                        result = []
                        for index in range(params['batchsize'] // 4):
