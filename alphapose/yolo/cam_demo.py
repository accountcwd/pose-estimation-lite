...
def fast_detect(ret_point):
    min_x = float(imgResW)
    min_y = float(imgResW)
    max_x = float(0)
    max_y = float(0)
    
    for point in ret_point['cord']:
    ...
    w = max_x-min_x
    h = max_y-min_y
    min_x = max(min_x - w*0.2, 0)
    min_y = max(min_y - h*0.2, 0)
    max_x = min(max_x + w*0.2, imgResW)
    max_y = min(max_y + h*0.2, imgResH)
...
def vis_frame_fast(frame, res, format='coco'):
    ...
    for human in res:
        ...
        for n in range(kp_scores.shape[0]):
            ...
            ret_point['cord'].append((kp_preds[n, 0], kp_preds[n, 1]))
            ret_point['sc'].append(kp_scores[n])
            ...
            try:
                ...
                if ret:
                    ...
                    if flag:
                        ...
                        det_count +=1
                        print('detection runtimes:%d', det_count)
                    else:
                        bbox = fast_detect(ret_point)
                        
                    inps, pt1, pt2 = crop_from_dets(im_to_torch(frame), bbox)
                    ...
                    flag = False if pred_cls[0][0]>pred_cls[0][1] else True
                    ...
                    vis_img, ret_point = vis_frame_fast(frame, result)
                    if len(ret_point['cord'])<=3:
                        flag = True 
                    # print(ret_point)
